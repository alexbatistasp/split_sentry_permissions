...
nome_do_db = str(sys.argv[3])
print("NomedoDB ==> " +nome_do_db)
...
while contador <  len(parsed['dbPolicies']):    
    if(str(parsed['dbPolicies'][contador]['resource']['database']) == nome_do_db ):
        ...
        if (int(len(new_file['dbPolicies'])) == int(qtd_por_file)):        
            escreve_novo_json(new_file,nome_do_db+"_"+str(numero_file))        
            numero_file+=1
            new_file = novo_dict()
    contador+=1
escreve_novo_json(new_file,nome_do_db+"_"+"final")
##print("====depois====")
##print(json.dumps(new_file))
